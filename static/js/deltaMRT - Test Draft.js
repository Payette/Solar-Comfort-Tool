...
//change 1 from cbe, fbes as 0.55, result different almost 0
// window.SOLAR_COMFORT.ERF_solar_f = (feff, fsvv, Idir, beta, fp, SHGC, alpha_sw, f_bes_f) => {
//     return ((0.5 * feff * fsvv * (0.6 *Idir * Math.sin(beta * DEG_TO_RAD)))
//     + (0.55 * feff * fp * Idir) + (0.16 * feff * fsvv * Idir))
//     * SHGC
//     * (alpha_sw / 0.95);
// }


//change 2 from cbe, fbes, o.16=0.2(I_dir)
////////////////////////////////////////////////////////////////////////////////////////////////////////
// window.SOLAR_COMFORT.ERF_solar_f = (feff, fsvv, Idir, beta, fp, SHGC, alpha_sw, f_bes) => {
//     return ((0.5 * feff * fsvv * (0.6 *Idir * Math.sin(beta * DEG_TO_RAD)))
//     + (f_bes * feff * fp * Idir) + (0.16 * feff * fsvv * Idir))
//     * SHGC
//     * (alpha_sw / 0.95);
// }

///change 3 from article, Idiff=0.17*Idir*sin beta
window.SOLAR_COMFORT.ERF_solar_f = (feff, fsvv, Idir, beta, fp, SHGC, alpha_sw, f_bes) => {
    //let Idiff = 0.17 * Idir * Math.sin(beta * DEG_TO_RAD)
    let Idiff = 0.2 * Idir
    console.log (`Idir: ${Idir}`,`Idiff: ${Idiff}`,`fsvv: ${fsvv}`)

    //result delta mrt 0-2.2: Idir:8; Idiff:161
    // return (0.5 * feff * fsvv * (161 + 0.6*(8 * Math.sin(33.75 * DEG_TO_RAD) + 161))
    // + (feff * fp * f_bes * 8))
    // * SHGC
    // * (alpha_sw / 0.95);

    //result delta mrt 8.5-16.7: Idir:559; Idiff:216
    // return (0.5 * feff * fsvv * (161 + 0.6*(559 * Math.sin(33.75 * DEG_TO_RAD) + 261))
    // + (feff * fp * f_bes * 559))
    // * SHGC
    // * (alpha_sw / 0.95);
...
window.SOLAR_COMFORT.f_bes_f = (d, j, beta, solar_azimuth, sillHeight, h, w, room_width) => { 
    ...
    if ( ((room_width_si - w_si)/2 - d_si/tan_azi) < j_si && j_si < ((room_width_si + w_si)/2 - d_si/tan_azi) ){
        ...
        //     // if (dia < (sillHeight_si / tan_beta)) {
        //     //     return 0.5;
        //     //     //return Math.min(1, Math.max(0, (1.23 + dia * tan_beta - sillHeight_si)/1.23));
        //     // } else if (dia >= (sillHeight_si / tan_beta) && dia < ((sillHeight_si + h_si) / tan_beta)){
        //     //     return 1;
        //     //     //return Math.min(1, Math.max(0, (h_si - dia * tan_beta + sillHeight_si)/1.23));
        //     // } else if (dia >= (sillHeight_si + h_si) / tan_beta) {
        //     //     return 0;
        //     // }
        // }
...
// window.SOLAR_COMFORT.f_bes_f = (d, beta, sillHeight, h) => { 
//     if (posture === "standing") {
//         if ((1.73 + d * Math.tan(beta) - to_m(sillHeight)) > (h + to_m(sillHeight))) {
//             return Math.min(1, Math.max(0, (2 * to_m(sillHeight) + h - d * Math.tan(beta))/1.73));
//         }
//         return Math.min(1, Math.max(0, (1.73 + d * Math.tan(beta) - to_m(sillHeight))/1.73));
//     } else if (posture === "seated" || (1.23 + d * Math.tan(beta) - to_m(sillHeight)) > (h + to_m(sillHeight)) ){
//         return Math.min(1, Math.max(0, (2 * to_m(sillHeight) + h - d * Math.tan(beta))/1.23));
//     } else {
//         return Math.min(1, Math.max(0, (1.23 + d * Math.tan(beta) - to_m(sillHeight))/1.23));
//     }
// }

// if (posture === "standing") {
//     if ((1.73 + dia * Math.tan(radians) - sillHeight_si) > (h_si + sillHeight_si)) {
//         return Math.min(1, Math.max(0, (2 * sillHeight_si + h_si - dia * Math.tan(radians))/1.73));
//     }
//     return Math.min(1, Math.max(0, (1.73 + dia * Math.tan(radians) - sillHeight_si)/1.73));
// } else if (posture === "seated" || (1.23 + dia * Math.tan(radians) - sillHeight_si) > (h_si + sillHeight_si) ){
//     return Math.min(1, Math.max(0, (2 * sillHeight_si + h_si - dia * Math.tan(radians))/1.23));
// } else {
//     return Math.min(1, Math.max(0, (1.23 + dia * Math.tan(radians) - sillHeight_si)/1.23));
// }
///////////////////////////////////////////////////////////////////////////////////////////////////////
