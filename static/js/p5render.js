...
document.getElementsByName(`studyType`).forEach(e => e.addEventListener('click', () => {
  if (e.id === 'ppdRadio') {
    ...
    document.getElementById("occupancyWrapper").classList.add('hidefromall');

    document.getElementById("thermalcomfortvisualizationsettings").classList.remove('hidefromall');
    thermalComfortVisualizationChangeHandler();
  } else {
    ...
    document.getElementById("occupancyWrapper").classList.remove('hidefromall');
    
    document.getElementById("thermalcomfortvisualizationsettings").classList.add('hidefromall');
    Array.from(document.getElementsByClassName("thermalcomfortlegend")).forEach(e => e.classList.add('hidefromall'));
  }
}))
...
function thermalComfortVisualizationChangeHandler() {
  let thermalComfortVisualization = document.getElementById(`thermalcomfortvisualization`).value;

  document.getElementById("ppdlegend").classList.add('hidefromall');
  document.getElementById("mrtlegend").classList.add('hidefromall');
  ...
  } else if(thermalComfortVisualization === THERMAL_COMFORT_VIZ_PMV) {
    document.getElementById("pmvlegend").classList.remove('hidefromall');
  } else if(thermalComfortVisualization === THERMAL_COMFORT_VIZ_MRT) {
    document.getElementById("mrtlegend").classList.remove('hidefromall');
  } else if(thermalComfortVisualization === THERMAL_COMFORT_VIZ_DELTAMRT) {
    document.getElementById("deltamrtlegend").classList.remove('hidefromall');
...
// https://andrewringler.github.io/palettes/#/11|d|006ce1,eeffee|d60059|1|1|0|
let mrtColors50to100 = ['#006ce1', '#5f87e5', '#8aa3e8', '#aec1eb', '#cfdfed', '#eeffee', '#fcbdcd', '#f386a1', '#dc5274', '#b62448', '#83001f'];
let mrtDataLabels = [50, 75, 100];

// https://andrewringler.github.io/palettes/#/9|d|0089e2,6ba871|6ba871,ff0045|1|1|0|
let pmvColorsNeg3to3 = ['#006ce1', '#5f87e5', '#8aa3e8', '#aec1eb', '#cfdfed', '#eeffee', '#fcbdcd', '#f386a1', '#dc5274', '#b62448', '#83001f'];
...
function ppdLegend() {
  ...
  container.setAttribute('class', 'thermalcomfortlegend')

  let label = document.createElement("span");
  label.setAttribute("class", "legendlabel");
  label.innerHTML = 'Predicted Percentage of Dissatisfied (PPD) <sup id="fnref:11"><a href="#fn:11" rel="footnote">?</a></sup>';
  ...
  var vis = d3.select("#ppdlegend svg")
  let numBlocks = 15
...
// MRT Legend
function mrtLegend(id, name, colorScale, colorList, dataLabels) {
  let svgWidth = 327;

  let container = document.createElement("div");
  container.setAttribute("id", id);
  container.setAttribute('class', 'thermalcomfortlegend hidefromall');
  ...
  d3.select(`#${id}`).append('svg').attr("height", 80).attr("width", svgWidth)
  var vis = d3.select(`#${id} svg`)
  let numBlocks = colorList.length;
  let width = svgWidth - 5;
  var arr = d3.range(numBlocks)
  var dataset = dataLabels;
  let dmin = dataset[0];
  let dmax = dataset[dataset.length-1];
  let drange = parseFloat(dmax - dmin);
  let arrToData = v => v/(numBlocks-1)*drange+dmin;
  ...
  vis.selectAll('rect').data(arr).enter()
    ...
    .attr({
      ...
      width: blockWidth,
      stroke: d3.rgb(0, 0, 0),
      fill: function (d) { return colorScale(arrToData(d)) }
    });
  ...
  vis.selectAll("text") // <-- Note "text", not "circle" or "rect"
    ...
    .attr({
      x: function (d, i) {
        if (i === 0) return 12;
        if (i === dataset.length - 1) return width - 10;
        return Math.ceil((d-dmin)/drange*numBlocks) / numBlocks * width - 10;
      },
      y: 40
    });
}
  ...
  return function (p) {
    ...
    p.setup = function () {
      ...
      p.cnv.mouseOut(() => {
        ...
        document.getElementById(`visualizationhoverstats${c}`).classList.add('hidefromall');
      });
      ...
      document.getElementsByName("vShadeStart")[0].addEventListener('select', p.reload);
    ...
    p.draw = function () {
      ...
      let roomOrientationValueNeg = (360 - roomOrientationValue - 47) % 360 /* we draw the room at a ~47degree offset?? */;
      ...
      for (let i = 0; i < r.glzCoords.length; i++) {
        ...
        if(arrayX[0] !== arrayX[1]) { // hide 0-width windows
          // Draw window frame (glazing)
      ...
      for (let i = 0; i < r.glzCoords.length; i++) {
        ...
        if (window.SOLAR_COMFORT[`settings${c}`].vertShadeStart == "R") {
      ...
      for (let k = 0; k < solarCoordinates.length; k++) {
        let windowOrientation = (roomOrientationValue + 180) % 360;
        let windowOrientationAdjustedSolarCoord = (solarCoordinates[k][0] - p.float(windowOrientation)) % 360;
        solarCoordinatesRoomOrientationAdjusted.push(windowOrientationAdjustedSolarCoord);
      }
      ...
      if (annualOn) {
        ...
        window.SOLAR_COMFORT[`windowSolarCoolingLoad${c}`] = round1Decimal(windowSolarCoolingLoad);
        ...
        if (window.SOLAR_COMFORT[`settings${c}`].thermalComfortSingleHour && solarCoordinates.length > 0) {
          ...
          let deltaMRT_grid = window.SOLAR_COMFORT.calculateDeltaMRT_for_Grid(
            ...
            window.SOLAR_COMFORT[`settings${c}`].Posture, window.SOLAR_COMFORT[`settings${c}`].windowHeightValue, window.SOLAR_COMFORT[`settings${c}`].windowWidthValue,
            elevation,
            azimuth,
            window.SOLAR_COMFORT[`settings${c}`].shgc,
            window.SOLAR_COMFORT[`settings${c}`].asa,
            geoResult,
            window.SOLAR_COMFORT[`settings${c}`].gridHeightValue
          );
      ...
      for (let i = 0; i < gridX; i++) {
        ...
        for (let j = 0; j < gridY; j++) {
          ...
          if (window.SOLAR_COMFORT[`settings${c}`].thermalComfortSingleHour && window.SOLAR_COMFORT[`MRTGrid${c}`]) {
            ...
            if(window.SOLAR_COMFORT.settings.thermalComfortVisualization === THERMAL_COMFORT_VIZ_PPD) {
            ...
            } else if(window.SOLAR_COMFORT.settings.thermalComfortVisualization === THERMAL_COMFORT_VIZ_PMV && !isNaN(mrtValues.pmv)) {
              gridColor = pmvColorScaleNeg3to3(mrtValues.pmv);
